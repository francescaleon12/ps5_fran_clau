...
Step 1: Check if the year is 2013 or later. If not, ask the user to enter a valid year.

Step 2: Initialize empty lists for storing titles, links, dates, and categories.

Step 3: Start web scraping as in Step 1, using a loop to go through each page.
...
Step 5: If the earliest date on the current page is earlier than the user-specified start date, break the loop.

Step 6: After the loop, create a DataFrame from the lists.

Step 7: Filter the DataFrame to keep only records with dates from the user-specified date onward.

Step 8: Save the final DataFrame to a CSV file.
